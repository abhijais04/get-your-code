...
def downloadFile(row, code_directory):
    ...
    contestName = getContestName(contestId)
...
def getAllAcceptedSubmissions(username):
...
def downloadAllSolutions():
    try:
        ...
        rows = getAllAcceptedSubmissions(username)

        # Iterate through all submission and download source-code for them
        for row in rows:
            downloadFile(row, code_directory)
...
# getAllAcceptedSubmissions("abhi04code")
# getContestName("479")